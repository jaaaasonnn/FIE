...
*Note: this skill's own Block Library and image-generation/logo-fetching steps assume a general marketing/portfolio web build. On a real-estate marketplace like FieGH, the "OUT OF SCOPE" section above matters: apply this skill's rules fully to marketing surfaces (landing page, host-signup page, about/how-it-works pages) and apply only the universal hygiene rules (contrast, em-dash ban, button/CTA discipline, color/shape consistency, dark mode, reduced motion, real images) to dashboard and multi-step booking/listing flows, rather than the landing-page-specific layout rules (hero stack discipline, eyebrow restraint, bento cell counts) which don't apply to dense product UI.*
