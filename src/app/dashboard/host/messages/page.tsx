...
function groupConversations(messages: ApiMessage[], userId: string): Conversation[] {
  ...
  for (const [key, msgs] of map) {
    ...
    conversations.push({
      ...
      guestName:   other.name ?? 'Guest',
      guestPhoto:  other.profilePhoto,
      ...
      messages: msgs.map((m) => ({
        id:   m.id,
        from: m.senderId === userId ? 'host' : 'guest',
        text: m.content,
...
export default function HostMessagesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/messages?userId=${user.id}`)
      ...
      .then((data) => {
        ...
        setActiveConv((prev) => {
          if (prev) return grouped.find((c) => c.id === prev.id) ?? grouped[0] ?? null
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    try {
      ...
      setActiveConv((prev) => {
        if (!prev) return prev
        return { ...prev, lastMessage: chatMsg.text, time: 'Just now', messages: [...prev.messages, chatMsg] }
      })
      setConversations((prev) =>
        prev.map((c) =>
          c.id === activeConv.id
            ? { ...c, lastMessage: chatMsg.text, time: 'Just now', messages: [...c.messages, chatMsg] }
            : c,
        ),
      )
      setNewMsg('')
